import { useEffect, useState } from 'react'
import api from '../../../services/api'
import InfoCandidato, { InfoCandidatoProps } from '../../InfoCandidato'

type DeputadoEstadualPageProps = {
  nomeCandidato: string
  estado: string
}

function DeputadoEstadualPage({ nomeCandidato, estado }: DeputadoEstadualPageProps) {
  const [candidatos, setCandidatos] = useState()
  const [urlBusca, setUrlBusca] = useState(`/deputados-estaduais/${estado}`)

  const valorAtual = nomeCandidato
    ? `/deputados-estaduais/${estado}/${nomeCandidato}`
    : `/deputados-estaduais/${estado}`

  if (urlBusca !== valorAtual) {
    setUrlBusca(valorAtual)
  }

  useEffect(() => {
    api
      .get(urlBusca)
      .then((response) => {
        setCandidatos(response.data)
      })
      .catch((err: Error) => {
        console.error(`Não foi possível consultar a API :/\nErro: ${err}`)
      })
  }, [urlBusca, estado])

  const candidatosSafe: InfoCandidatoProps[] = candidatos ? candidatos : []

  candidatosSafe.forEach((candidato) => {
    candidato.tipoCandidato = 'deputados-estaduais'
    candidato.estado = estado
  })

  return (
    <>
      {candidatosSafe.map((candidato) => (
        <InfoCandidato {...candidato} key={candidato.id} />
      ))}
    </>
  )
}

export default DeputadoEstadualPage
